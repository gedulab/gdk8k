...
#define GDK8_IRAM_ADDR			0xff098010
...
static int __init rockchip_grf_init(void)
{
	...
	struct regmap *grf;
	int ret, i;
	unsigned int valCon4, valIOMUX, valCon7;
	...
	regmap_read(grf, RK3328_GRF_SOC_CON4, &valCon4);
	regmap_read(grf, RK3328_GRF_GPIO1A_IOMUX, &valIOMUX);
	regmap_read(grf, RK3328_CRU_CLKGATE_CON7, &valCon7);

	pr_info("Old RK3328_GRF_SOC_CON4 0x%x, Old RK3328_GRF_GPIO1A_IOMUX: 0x%x, "
		"Old RK3328_CRU_CLKGATE_CON7 0x%x\n",
		valCon4, valIOMUX, valCon7);

	for (i = 0; i < grf_info->num_values; i++) {
		const struct rockchip_grf_value *val = &grf_info->values[i];

		pr_info("%s: adjusting %s in %#6x to %#10x, %d\n", __func__,
			val->desc, val->reg, val->val, grf_info->num_values);
	...
	pr_info("New RK3328_GRF_SOC_CON4 0x%x, New RK3328_GRF_GPIO1A_IOMUX: 0x%x, "
		"New RK3328_CRU_CLKGATE_CON7 0x%x\n",
		valCon4, valIOMUX, valCon7);
