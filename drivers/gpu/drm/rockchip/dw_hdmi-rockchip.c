...
#if 0
		// copied from qufu version by Raymond to fix the "failed to set" issue on 2022-1-28
		if (!state)
			ret = drm_atomic_helper_connector_set_property(connector,
									property, val);
		else
			ret = drm_atomic_connector_set_property(connector, state,
								 property, val);
#endif 
		/*DRM_ERROR("failed to set rockchip hdmi connector property %d\n", 
			hdmi->colorimetry_property);*/
	}
	return ret;
}
