...
#include <asm/io.h>
// echo "iram 0x8881" | sudo tee -a /proc/llaolao

static noinline long gdk_breakin(unsigned int sysnum, void *addr)
{
...
int gdk_check_breakin(void)
{
    int value;
    void __iomem* base;

    set_fixmap_io(FIX_EARLYCON_MEM_BASE, GDK8_IRAM_ADDR & PAGE_MASK);
    base = (void __iomem *)__fix_to_virt(FIX_EARLYCON_MEM_BASE);
    base += GDK8_IRAM_ADDR & ~PAGE_MASK;
    if(!base) {
    ...
    value = readl(base);
    printk("GDDBG: read GDK8_IRAM_ADDR with value: 0x%x\n", value);
    if(value >= NDB_NOTE_KERNEL_BASE && value < NDB_NOTE_KERNEL_BASE + 0x10000) {
	printk("GDDBG: waiting kernel debugger (%d)\n", value - NDB_NOTE_KERNEL_BASE);
	mdelay(value - NDB_NOTE_KERNEL_BASE);

	if((value % 2) == 0) {
	    printk("GDDBG: get debug kernel flag: 0x%x\n", value);
	    gdk_breakin(8, 0);
	}
    }
