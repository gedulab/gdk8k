...
static void dw_hdmi_update_power(struct dw_hdmi *hdmi)
{
	...
	dev_dbg(hdmi->dev, "dw_hdmi_update_power old: hdmi->initialized %d; hdmi->disabled %d; hdmi->bridge_is_on %d; hdmi->phy.enabled %d",
		hdmi->initialized, hdmi->disabled, hdmi->bridge_is_on, hdmi->phy.enabled);
	...
	if (force == DRM_FORCE_OFF) {
		...
		if (hdmi->bridge_is_on) {
			dev_dbg(hdmi->dev, "enter dw_hdmi_poweroff force: %d", force);
			dw_hdmi_poweroff(hdmi);
		}
	} else {
		if (!hdmi->bridge_is_on) {
			dev_dbg(hdmi->dev, "enter dw_hdmi_poweron force: %d", force);
			dw_hdmi_poweron(hdmi);
		}
	}
	
	dev_dbg(hdmi->dev, "dw_hdmi_update_power new: hdmi->initialized %d; hdmi->disabled %d; hdmi->bridge_is_on %d; hdmi->phy.enabled %d",
		hdmi->initialized, hdmi->disabled, hdmi->bridge_is_on, hdmi->phy.enabled);
}
		...
		const struct dw_hdmi_plat_data *plat_data)
{
	...
	printk("GDDBG: hdmi->initialized old %d", hdmi->initialized);
	hdmi->initialized = false;
	printk("GDDBG: hdmi->initialized new %d", hdmi->initialized);
	...
	printk("GDDBG: hdmi->initialized new %d; hdmi->disabled %d; hdmi->bridge_is_on %d; hdmi->phy.enabled %d",
		hdmi->initialized, hdmi->disabled, hdmi->bridge_is_on, hdmi->phy.enabled);
